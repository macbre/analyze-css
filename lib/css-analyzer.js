...
class CSSAnalyzer {
  constructor(options, CSSwhat) {
    this.options = options;
    this.CSSwhat = CSSwhat;
  ...
  initRules() {
    const debug = require("debug")("analyze-css:rules"),
      re = /\.js$/;
      ...
      switch (rule.type) {
        ...
        case "rule":
          ...
          rule.selectors.forEach(function (selector) {
            ...
            try {
              parsedSelector = this.CSSwhat.parse(selector);
            } catch (err) {
              /**
  ...
  run() {
    ...
    if (stylesheet && stylesheet.parsingErrors) {
      stylesheet.parsingErrors.forEach((err) => {
        this.emit("error", new Error(err.reason));
      });
    }
