...
analyzer.EXIT_CSS_PARSE_ERROR = 251;
	...
	parseCss: function(css) {
		...
		try {
			...
			this.tree = cssParser(css, {
				// errors are listed in the parsingErrors property instad of being thrown (#84)
				silent: true
			});
		} catch (ex) {
			var errMsg = 'CSS parsing failed: ' + ex.reason + ' @ ' + ex.line + ':' + ex.column;

			debug(errMsg);
			debug(ex);
			debug('Offending line: ' + css.split('\n')[ex.line - 1]);

			return this.error(errMsg, analyzer.EXIT_CSS_PARSE_ERROR);
		}
