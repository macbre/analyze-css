...
function analyzer(css, options, callback) {
  ...
  if (typeof this.options.preprocessor === "string") {
    debug('Using "%s" preprocessor', this.options.preprocessor);

    var preprocessor = preprocessors.get(this.options.preprocessor);

    try {
      css = preprocessor.process(css, this.options);
    } catch (ex) {
  ...
  if (this.options.noOffenders !== true) {
